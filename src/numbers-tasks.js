...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const sumVectors = x1 * x2 + y1 * y2;
  const q1 = Math.sqrt(y2 * y2 + x2 * 2);
  const q2 = Math.sqrt(y1 * y1 + x1 * x1);
  return Math.acos(sumVectors / (q1 * q2));
}
...
function roundToPowerOfTen(num, pow) {
  const powTen = 10 ** pow;
  const powNum = Math.round(num / powTen);
  const result = powTen * powNum;
  return result;
}
...
function getFibonacciNumber(index) {
  const arr = [0, 1];
  for (let i = 2; i <= index; i += 1) arr.push(arr[i - 1] + arr[i - 2]);
  return arr[index];
}
...
function getSumOfDigits(num) {
  const arr = String(num >= 0 ? num : -num).split('');
...
function getSine(num) {
  let res = 0;
  if (num !== 0) {
    res = Math.round(Math.sin(Math.PI / 2));
  }
  return res;
}
...
function isNumber(number) {
  let res = false;
  if (Number.isFinite(number)) res = true;
  return res;
}
...
function isInteger(number) {
  let res = false;
  if (Number.isInteger(number)) res = true;
  return res;
}
...
function getMaxNumber(firstNumber, secondNumber) {
  const arr = [firstNumber, secondNumber];
  return Math.max(...arr);
}
